#final_project_ics33.py

#Submitters:
# Partner 1(Name and ID): Grace Newman, 43355258
# Partner 2(Name anad ID): Bayan Tabbaa, 86148723
...
class Game:
    ...
    def __init__(self, player_amount, money=None):
        ...
        self.players_copy1 = self.players
        if len(bots_money_left) != 0:
            self.players = []
            for i in self.players_copy1:
...
class gui:
    ...
    #function to pack and create folding and betting buttons
    ...
    #ensures that the number of players is valid
    ...
    #sets up the game and destroys player amount entry prompt
    ...
    def betting_button_command(self):
        ...
        #self.players_money_left = players_money_left - self.bet_amount.get()
        self._destroy_move_buttons()
        self.round += 1
        if int(self.g.user[1]) > 0:
            self.betting_label[
                "text"] = "Insert the amount of money you would like to bet as a single integer, then press enter. "
    ...
    def _bot_decisions(self):
        ...
        self.players_money_left = self.g.user[1]
    ...
    #displays the community cards as images
    def _show_ccs(self):
