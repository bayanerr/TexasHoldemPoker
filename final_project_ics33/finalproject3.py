...
class Game:
    ...
    def __init__(self, player_amount):
        ...
        # player data structure is as follows:
        # [id, betting money, rank, card1, card2, cc1, cc2, cc3, cc4, cc5]
        ...
        for i in range(self.player_amount):
            self.players.append([i+1, 10, 10, random.choice(self.cards), random.choice(self.cards), self.c1, self.c2, self.c3, self.c4, self.c5])

    #This determines the rank of the players after round 1
    def determine_rank_r1(self):
        for i in self.players:
            if Royal_Flush(i[3:7]) == True: i[2] = 0
            elif Straight_Flush(i[3:7]) == True: i[2] = 1
            elif four_of_a_kind(i[3:7]) == True: i[2] = 2
...
def betting_button2(g, root, card_frame):
    ...
    #community_card_image4 = resize_cards(f'final_project_ics33/card_deck/{g.c4}.png')
    community_card_image4 = resize_cards('{}/card_deck/{}.png'.format(cwd, g.c4))
    community_card_4.config(image=community_card_image4)
    #community_card_image5 = resize_cards(f'final_project_ics33/card_deck/{g.c5}.png')
...
def checking_button(g, button_frame, root, bet_amount_get=0, bet_amount=None, betting_label=None, bet_amount_button=None):
    ...
    card_frame = LabelFrame(frame, text="  The Community Cards Are: ", bd=0)
...
def betting_button(g, button_frame, root):
    ...
    betting_label = Label(root,text="You currently have {}$ max to bet\ninsert the amount of money you would like to bet as a single integer, then press enter ".format(g.user[2]),padx=85, pady=4, bg="white", fg="black")
    betting_label.pack()
    ...
    bet_amount.insert(5, '')
    bet_amount_button = Button(root, text="enter", fg="pink", bg="black", command=lambda: checking_button(g, button_frame, root, bet_amount.get(), bet_amount, betting_label, bet_amount_button))
...
def player_setup(numb, root, title1, player_amount, player_amount_button):
    ...
    player_image = resize_cards('{}/card_deck/{}.png'.format(cwd, g.user[3]))
    player_image2 = resize_cards('{}/card_deck/{}.png'.format(cwd, g.user[4]))
